...
lambda = lambda_generate(lambda0, C_lam, mean_q, q_taker0, delta_s, TimeHorizon)
plot(lambda, type="l")
...
init_LOB = function(Na, alpha, LOB_0, q_provider, lambda0, C_lam, mean_q, q_taker0, delta_s){
  ...
  lambda = lambda_generate(lambda0, C_lam, mean_q, q_taker0,delta_s, Num_orders = num_LOs) # SLOW
...
execute_single_MO = function(q_taker_k, myLO_Books){
  ...
  if (u < q_taker_k){ # market buy order hits best ask
    ...
    myLO_Books[[1]][1,2] = myLO_Books[[1]][1,2] - 1 # subtract 1 from count of best ask
...
Sim_OB = function(lamda0, C_lam, mean_q, q_taker0, delta_s, TimeHorizon, Na, alpha, mu, myLOB_init, q_provider){
  lambda = lambda_generate(lambda0, C_lam, mean_q, q_taker0, delta_s, Num_orders = TimeHorizon)
...
simLOB = Sim_OB(lamda0, C_lam, mean_q, q_taker0, delta_s, TimeHorizon=250, Na, alpha, mu, myLOB_init, q_provider)
